...
3. `import` in Java is simply a way to tell Java the full name of the class. Unlike in C/C++'s `include`, `import` does **not** make the program larger. Plus, we always get the java.lang package pre-imported. Plus, we shouldn't use `import` if the classes are in the same package.

...
2. `import` in Java is simply a way to tell Java the full name of the class. Unlike in C/C++'s `include`, `import` does **not** make the program larger. Plus, we always get the java.lang package pre-imported. What's more, we shouldn't use `import` if the classes are in the same package.

3. `Arrays.asList(foo)` and `.toArray(new int[0])`
...
5. When using an interface, we usually use implementations like: `List<T> = new ArrayList<T>()`, `Queue<T> = new LinkedList<T>()`

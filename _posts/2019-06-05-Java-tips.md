...
* [Java API (the Java Library)](#java-api-the-java-library)
...
24. `return new int[]{1, 2}`.

25. `.length` for primitive types (e.g. int[], String[]), `.length()` for objects (e.g. String). And `.size()` for collections.

26. `String string = String.valueOf(charArray)` converts a char array to a string.
...
3. `import` in Java is simply a way to tell Java the full name of the class. Unlike in C/C++'s `include`, `import` does **not** make the program larger. Plus, we always get the java.lang package pre-imported. What's more, we shouldn't use `import` if the classes are in the same package.

4. [Java Collections](https://www.javatpoint.com/collections-in-java)

5. `Arrays.asList(foo)` and `.toArray(new int[0])`
...
7. When using an interface, we usually use implementations like: `List<T> = new ArrayList<T>()`, `Queue<T> = new LinkedList<T>()`
